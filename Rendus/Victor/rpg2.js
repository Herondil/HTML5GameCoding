...
console.log("Bonjour et bienvenu sur le simulateur de combat ultra réaliste: \"L'Elisium\"\n"+
	"Chaque sujet peut réaparaitre trois fois. Si un joueur meurt, il perd un item aléatoire de son inventaire. "+
	"Si un sujet réussi a se suicider, il garde son équipement...\n\nVoici une liste des actions possible"+
	"\n'recherche' > Permet de chercher un équipement dans l'Elisium. Il y a 50% de chance d'en trouver un. "+
	...
	"Si la cible est vous même, le sabotage réussira obligatoirement."+
//	"\n'protéger' > Cette action n'est pas encore au point..."+
...
var Sheepok = new player('Sheepok',3,		150,	2,	8,	3);
var Arendil = new player('Arendil',3,		210,	2,	3,	8);
var Leubogos= new player('Leubogos',3,		200,	3,	5,	4);
...
*/
		case "recherche":
			textFeedBack+=recherche(nom_player);
		break;
		...
		default:
			...
			"\nliste des actions: 'attaquer', 'recherche', 'saboter', 'inventaire'"+
			"\nnom_cible est facultatif pour les actions 'recherche' et 'inventaire'";
			tourPass=false;
	}
...
function recherche(nom_player)
{
	var result ="Vous avez une chances sur 2 de trouver un item\n";
...
function saboter(nom_player,nom_cible)
{
	if(nom_cible.inventaire.length>0)
	{
		...
		{
			i = Math.round(Math.random()*nom_cible.inventaire.length);
		}
		else
		{
